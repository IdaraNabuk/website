...
This is the entry point of the code and defines the return value which is of type unit (). This line defines the value of the expressions on the right. It's a pattern match of the form ```let pattern = expression```. ```Bogue.run``` will return a unit ```()```  and this will match with the unit ```()``` on the left and the compiler will be happy and we will have our side-effects from Bogue.run.
